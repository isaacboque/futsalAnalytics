"""Tests for TeamClassifier, BallTracker, and TacticalBoard (no GPU/camera required)."""
...
class TestBallTracker:
    ...
    def test_smoothing_averages_positions(self):
        ...
        # Mean of [0,0], [100,100], [200,200] = [100,100]
        np.testing.assert_allclose(result, [100.0, 100.0])

    def test_large_jump_resets_history(self):
        bt = BallTracker(max_distance=50)
        bt.update(np.array([0.0, 0.0]))
        # Jump far beyond max_distance — history is reset before this point is added
        result = bt.update(np.array([1000.0, 1000.0]))
        # After reset, only the new point is in history
        np.testing.assert_allclose(result, [1000.0, 1000.0])

    def test_none_clears_last_pos(self):
        bt = BallTracker()
        bt.update(np.array([50.0, 50.0]))
        bt.update(None)
        assert bt.last_pos is None
...
# ---------------------------------------------------------------------------
# TeamClassifier (colour extraction only — no KMeans fitting)
# ---------------------------------------------------------------------------
...
class TestTacticalBoard:
    ...
    def test_referee_not_drawn(self):
        """A referee (team_id=-1) should not change the board vs no players."""
        board = TacticalBoard(700, 350)
        img_no_players = board.draw_state([], None)
        img_referee = board.draw_state([(99, np.array([350.0, 175.0]), -1)], None)
        np.testing.assert_array_equal(img_no_players, img_referee)
